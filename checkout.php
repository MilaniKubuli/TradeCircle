...
if (is_post()) {
    ...
    $stmt = $pdo->prepare('INSERT INTO orders (buyer_id, buyer_name_snapshot, buyer_email_snapshot, total_amount, delivery_method, delivery_address, buyer_notes) VALUES (?, ?, ?, ?, ?, ?, ?)');
    $stmt->execute([
        $user['id'],
        $user['full_name'],
        $user['email'],
    ...
    $itemStmt = $pdo->prepare('INSERT INTO order_items (order_id, listing_id, listing_title_snapshot, seller_id, seller_name_snapshot, seller_email_snapshot, quantity, unit_price) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
